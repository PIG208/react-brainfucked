...
test("IOStream write", () => {
  ...
  expect(stream.buffer).toEqual(stringToASCIIs("addd"));
  expect(stream.buffer.length).toEqual(4);

  stream = ioReducer(stream, { type: "write", data: stringToASCIIs("k") });
  expect(stream.buffer).toEqual(stringToASCIIs("adddk"));
  expect(stream.buffer.length).toEqual(5);
});
...
test("IOStream seek", () => {
  ...
  expect(ASCIIsToString(stream.buffer)).toEqual("akqweasd123");

  stream = ioReducer(stream, { type: "seek", data: 2 });
  expect(stream.pointer).toEqual(2);
  expect(ioReducer(stream, { type: "read", data: 4 }).readBuffer).toEqual(stringToASCIIs("qwea"));
});
